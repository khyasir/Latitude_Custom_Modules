...
class CustomerPayment(models.Model): 
	...
	def load_invoice(self):
		...
		if self.partner_id:
			if self.receipts == True:
				...
				invoices = invoices.sorted(key=lambda r:r.invoice_date)
	...
	def journal_entry_with_tax_payments(self,debit_account,credit_account,debit_amount,credit_amount):
		...
		journal_entries = self.env['account.move'].search([('id','=',self.journal_entry_id.id)])
		for line in journal_entries.line_id:
			if line.identify=='d':
				line.credit = 0
		...
		for x in self.tax_link:
			if self.receipts == True:
				...
				create_tax = journal_entries.line_id.create({
				'account_id':x.payment_link.partner_id.property_account_receivable.id,    
				'partner_id':self.partner_id.id,
				'name':x.name,
				'credit':x.amount,
				'debit' :0,
				...
				create_tax = journal_entries.line_id.create({
				'account_id':x.payment_link.partner_id.property_account_payable.id,    
				'partner_id':self.partner_id.id,
				'name':x.name,
				'credit':0,
				'debit' :x.amount,
	...
	def journal_entry_with_tax(self):
		if self.receipts == True:
			...
			credit_account = self.partner_id.property_account_receivable.id
			debit_amount = self.amount
			credit_amount = self.amount
		else:
			debit_account = self.partner_id.property_account_payable.id
